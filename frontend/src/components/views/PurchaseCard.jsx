...
const PurchaseCard = ({purchase, index}) => {
  let sum = 0;
  purchase[0].basket.forEach((element) => {
    sum+=element.price
  })
    return (
  ...
  <div className="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
    ...
    <div className="flex items-center">
      <div className="text-sm">
        <p className="text-gray-900 leading-none">{purchase[0].seller.email}</p>
        <p className="text-gray-600">Кто продал - {purchase[0].date_sell}</p>
      </div>
    </div>
    <Link to={"/purchase/" + purchase[0].id}>
